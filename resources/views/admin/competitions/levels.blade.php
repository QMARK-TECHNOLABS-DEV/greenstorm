                ...
                <div class="p-2 ">
                    <form class="form-layout p-4 bd" id="create-stage-form" action="{{ route('stages.store') }}">
                      @csrf
                        ...
                        <h5 class="card-title tx-uppercase">Create New Stage for Competition ( {{ $competition->title }} ) </h5>
                        ...
                        <div class="row">
                          ...
                          <div class="col-sm-4 mg-t-10 mg-sm-t-0">
                            <input type="text" name="title" class="form-control" placeholder="Enter Stage Title">
                          </div>
                        </div><!-- row -->
		...
		function loadStages(){
			...
			$.ajax({
				...
				success: function (response) {
					...
					console.log(response); // Just for demonstration, you can customize this part as per your needs
					let html = '';
					if(response.stages.length >0){
						response.stages.forEach(function (stage) {
                            // if(stage.type != 'elimination' && stage.status == false){
